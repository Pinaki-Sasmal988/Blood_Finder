...
public class UpdateBankView extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			{
				String query="update blood_bank set ?='?' where bank_id=?";
				psmt=con.prepareStatement(query);
				psmt.setString(1, attribute_name);
				psmt.setString(2, new_data);
				psmt.setInt(3, Integer.parseInt(bank_id));
			}
			else
			{
				String query="update blood_details set ?='?' where bank_id='?'";
				psmt=con.prepareStatement(query);
				psmt.setString(1, attribute_name);
				psmt.setString(2, new_data);
				psmt.setString(3, bank_id);
			}
